...
class FirstFit final : public Policy {
public:
    FirstFit(const int w, const int servers, const int classes, const std::vector<unsigned int>& sizes) :
    ...
    std::unique_ptr<Policy> clone() const override {
        return std::make_unique<FirstFit>(w, servers, state_buf.size(), sizes);
    }
    explicit operator std::string() const override {
        return "FirstFit(servers=" + std::to_string(servers) + ", classes=" + std::to_string(state_buf.size()) +
