...
class StaticMSF final : public Policy {
public:
    StaticMSF(const int w, const int servers, const int classes, const std::vector<unsigned int>& sizes) :
        state_buf(classes), state_ser(classes), stopped_jobs(classes), ongoing_jobs(classes), freeservers(servers),
        servers(servers), w(w), sizes(sizes), violations_counter(0), current_class(-1), prev_class(-1), switching_time(false) {}
    ...
    ~StaticMSF() override = default;
    std::unique_ptr<Policy> clone() const override {
        return std::make_unique<StaticMSF>(w, servers, state_buf.size(), sizes);
    }
    explicit operator std::string() const override {
        return "StaticMSF(servers=" + std::to_string(servers) + ", classes=" + std::to_string(state_buf.size()) +
