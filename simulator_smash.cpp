...
public:
    ...
    const std::vector<int>& get_state_ser() { return state_ser; };

    const std::vector<int>& get_state_buf() { return state_buf; }

    const std::vector<std::list<int>>& get_stopped_jobs() { return stopped_jobs; }

    const std::vector<std::list<int>>& get_ongoing_jobs() { return ongoing_jobs; }

    int get_free_ser() { return freeservers; }

    int get_violations_counter() { return violations_counter; }
...
public:
    Simulator(const std::vector<double>& l, const std::vector<double>& u, const std::vector<int>& sizes, int w,
              int servers, int sampling_method) : ru(0.0, 1.0)
    {
        this->l = l;
        this->u = u;
        this->n = servers;
    ...
    void produce_statistics(std::vector<Confidence_inter>& occ_buff, std::vector<Confidence_inter>& occ_ser,
                            std::vector<Confidence_inter>& throughput, std::vector<Confidence_inter>& waitTime,
                            std::vector<Confidence_inter>& respTime, std::vector<bool>& warnings,
                            Confidence_inter& wasted, Confidence_inter& violations, Confidence_inter& utilisation,
                            Confidence_inter& occ_tot, Confidence_inter& wait_tot, Confidence_inter& resp_tot,
                            Confidence_inter& timings_tot, Confidence_inter& big_sequence,
                            Confidence_inter& big_seq_amount, Confidence_inter& max_big_seq_len,
                            Confidence_inter& small_sequence, Confidence_inter& small_seq_amount,
                            Confidence_inter& max_small_seq_len, Confidence_inter& nstar_small,
                            Confidence_inter& nstar_big, Confidence_inter& idle_counter, Confidence_inter& idle_prob,
...
void run_simulation(Experiment e, unsigned long events, int repetitions,
                    ...
                    Confidence_inter* res_nstar_small, // out: avg. number of small jobs in the queue at the beginning
                                                       // of a small class busy period
                    Confidence_inter* res_nstar_big, // out: avg. number of big jobs in the queue at the beginning of a
                                                     // big class busy period
...
)
{
    ...
    sim.produce_statistics(*res_buf, *res_ser, *res_th, *res_wait, *res_resp, *res_warn, *res_waste, *res_viol,
                           *res_util, *res_buf_tot, *res_wait_tot, *res_resp_tot, *res_timings, *res_big_seq,
                           *res_amount_big_seq, *res_big_seq_max_len, *res_small_seq, *res_amount_small_seq,
                           *res_small_seq_max_len, *res_nstar_small, *res_nstar_big, *res_idle_counter, *res_idle_prob);
}
...
int main(int argc, char* argv[])
{
    ...
    for (int w = 1; w <= 6; w++)
    {
        ...
        std::cout << "*** Processing - N: " << std::to_string(n) << " - Policy: " << std::to_string(w)
                  << " - Type: " << type << " - Sampling: " << sampling_name[sampling_method] << " ***" << std::endl;
        ...
        std::string lambdas_filename ="Inputs/" + cell + "/arrRate_" + type + ".txt"; //"Inputs/arr_combined.txt";
        "Inputs/smaller_arrRate.txt"; read_lambdas(lambdas_filename, arr_rate);*/
        ...
        std::string out_filename = "simulation_results_N" + std::to_string(n) + "_T" + std::to_string(sizes[1]) + "_W" +
            std::to_string(w) + "_" + sampling_name[sampling_method] + "_ps" + std::to_string(p[0]) + "_mus" +
            std::to_string(1 / mus[0]) + "_mub" + std::to_string(1 / mus[1]) + ".csv";
        ...
        {
            threads[i] = std::thread(run_simulation, ex[i], n_evs, n_runs, &res_buf[i], &res_ser[i], &res_th[i],
                                     &res_wait[i], &res_resp[i], &res_warn[i], &res_waste[i], &res_viol[i],
                                     &res_util[i], &res_buf_tot[i], &res_wait_tot[i], &res_resp_tot[i], &res_timings[i],
                                     &res_big_seq[i], &res_amount_big_seq[i], &res_big_seq_max_len[i],
                                     &res_small_seq[i], &res_amount_small_seq[i], &res_small_seq_max_len[i],
                                     &res_nstar_small[i], &res_nstar_big[i], &res_idle_counter[i], &res_idle_prob[i]);
        }
        for (int i = 0; i < ex.size(); i++)
        {
            threads[i].join();
        ...
        headers.insert(headers.end(),
                       {"Wasted Servers",
                        ...
                        "Exe. Time ConfInt",
                        "Big Sequence Lenght",
                        "Big Sequence Lenght ConfInt",
                        "Big Sequence Amount",
                        "Big Sequence Amount ConfInt",
                        "Max Big Sequence Length",
                        "Max Big Sequence Length ConfInt",
                        "Small Sequence Lenght",
                        "Small Sequence Lenght ConfInt",
                        "Small Sequence Amount",
                        "Small Sequence Amount ConfInt",
                        "Max Small Sequence Length",
                        "Max Small Sequence Length ConfInt",
                        "Nstar Big",
                        "Nstar Big ConfInt",
                        "Nstar Small",
                        "Nstar Small ConfInt",
                        "Idle Counter",
                        "Idle Counter ConfInt",
                        "Idle Prob",
                        "Idle Prob ConfInt"});
        ...
        for (int i = 0; i < ex.size(); i++)
        {
            ...
            for (int c = 0; c < sizes.size(); c++)
            {
                ...
                outputFile << "[" << res_buf.at(i).at(c).min + res_ser.at(i).at(c).min << ", "
                           << res_buf.at(i).at(c).max + res_ser.at(i).at(c).max << "]" << ";";
